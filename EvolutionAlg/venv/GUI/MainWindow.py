...
class App(ctk.CTk):
    ...
    def randomGeneratePoinrs(self):
        ...
        try:
            ...
            for i in range(random_count):
                ...
                if ((x, y, 0) not in self.points) and ((x, y, 0) not in self.points):
    ...
    def openWindowRangeRandow(self):
        ...
        submit_button = ctk.CTkButton(self.generate_random_window, text="Подтвердить", command=self.randomGeneratePoinrs)
            ...
            for x, y, value in csvreader:
                try:
                    ...
                    if value_int == 0 or value_int == 1:
                        self.points.append((x_int, y_int, value_int))
    ...
    def handlerAddPoint(self):
        ...
        try:
            ...
            y = int(y)
            value = int(value)
            if ((x, y, 0) not in self.points) and ((x, y, 0) not in self.points) and (value == 0 or value == 1):
                self.points.append((x, y, value))
                self.x_string.set('')
                self.y_string.set('')
